...
namespace Lesson29
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void UpdateForm(List<Nakladnaya> list)
        {
            ...
            {
                dataGridViewTovar.Columns[i].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            }
        ...
        private void dataGridViewTovar_ColumnHeaderMouseClick_1(object sender, DataGridViewCellMouseEventArgs e)
        {
        ...
        private void buttonWord_Click(object sender, EventArgs e)
        {
            if (dataGridViewTovar.SelectedRows.Count != 0)
            {
                ...
                range = doc.Content;
                range.Find.Execute("Цена", ReplaceWith: current.DateOrder);
                range = doc.Content;
                range.Find.Execute("ФИО поставщика", ReplaceWith: current.Provider);
                range = doc.Content;
                range.Find.Execute("ФИО получателя", ReplaceWith: current.Recipient);
                range = doc.Content;
